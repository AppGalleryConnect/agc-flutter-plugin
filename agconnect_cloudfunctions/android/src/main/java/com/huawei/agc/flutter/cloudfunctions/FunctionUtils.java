...
final class FunctionUtils {
    ...
    static FunctionCallable getFunctionCallable(@NonNull final MethodCall methodCall, @NonNull final AGConnectFunction agConnectFunction) {
        final String httpTriggerURI = methodCall.argument("httpTriggerURI");
        final long timeout = getTimeout(methodCall.argument("timeout"));
        final TimeUnit units = getTimeUnit(methodCall.argument("units"));

        final FunctionCallable functionCallable = agConnectFunction.wrap(httpTriggerURI);
